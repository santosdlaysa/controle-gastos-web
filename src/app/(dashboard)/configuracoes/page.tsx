  ...
  return (
    <div className="p-6 max-w-xl mx-auto">
      ...
      <div className="bg-surface rounded-xl p-4 border border-border mb-6">
        <div className="text-xs text-muted mb-1">Conta</div>
      ...
      <div className="bg-surface rounded-xl p-5 border border-border mb-4">
        ...
        <div className="space-y-3">
          {[
            { label: "Salário", value: salary, set: setSalary },
            { label: "Vale", value: vale, set: setVale },
            { label: "Outros", value: otherIncome, set: setOtherIncome },
          ].map(({ label, value, set }) => (
            <div key={label} className="flex items-center gap-3">
              <label className="text-xs text-muted w-16">{label}</label>
              <input
                type="number"
                min="0"
                step="0.01"
      ...
      {/* Total budget */}
      <div className="bg-surface rounded-xl p-5 border border-border mb-4">
        <h2 className="text-sm font-semibold text-foreground mb-4">Orçamento Total (mês atual)</h2>
        <input
          type="number"
          min="0"
          step="0.01"
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        className={`w-full py-3.5 rounded-xl font-semibold text-sm transition-colors ${saved ? "bg-success text-white" : "bg-brand text-white hover:bg-brand/90"} disabled:opacity-60`}
      >
